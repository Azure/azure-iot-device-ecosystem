...
device: openblocks iot vx2
language: c
---

Run a simple C sample on OpenBlocks IoT VX2 device running Debian GNU Linux 9
===
---
...
This document describes how to connect OpenBlocks IoT VX2 device running Debian GNU Linux 9 with Azure IoT SDK. This multi-step process includes:
...
4. Find the following place holder for editing protocol, if you want to chane from default:
...
6. Save your changes by pressing ESC :w and when nano prompts you to save it as the same file, just press ENTER.

7. Press ESC :q to exit vi.
...
### 3.2.3 Edit build.sh to reduce the amount of memory required for build

1.  Open the schell script file in a text editor

        sudo vi azure-iot-sdk-c/build_all/linux/build.sh

2. Find the following place holder for run make command

        make --jobs=$MAKE_CORES

3. Remove the optional arguments, '--jobs=$MAKE_CORES'

4. Save your changes by pressing ESC :w and when nano prompts you to save it as the same file, just press ENTER.

5. Press ESC :q to exit vi.

### 3.2.4 Build the samples:
